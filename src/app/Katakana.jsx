...
function Hiragana({
    ...
    const [questions, setQuestions] = useState(() => {
        ...
        return _.shuffle([...Katakana].slice(0, questionsCount));
    });
    const [questionIdx, setQuestionIdx] = useState(() => {
        // 問題の出題順を生成
        return _.shuffle(Array(questionsCount).fill().map((e, i) => i));
    });
    ...
    const questionTable = useMemo(() => {
        return _.chunk(questions.map(q => q ? q[1] : null), ITEM_PER_ROW);
    }, [questions]);
    ...
    return (
        <div className="katakana content">
            <div className="header">
                <div className="back">
                    <div className="button" onClick={onBack}>もどる</div>
                </div>
                <div className="timer">
                    <Timer start={start} />
                </div>
